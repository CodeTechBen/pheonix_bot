...
class Creature(commands.Cog):
    ...
    async def create_class(self,
        ...
        if not Validator.is_valid_class(class_name, is_playable):
            await ctx.send("Usage: !create_class <class_name> <True/False>")
            return

        response = generate_class(
            ctx.guild, class_name.title(), is_playable, conn)
    ...
    async def create_race(self,
        ...
        if not Validator.is_valid_class(race_name, is_playable):
